...
TEST_CASE("Test name", "[Tag1][Tag2]...")
{
	...
	std::vector<vector<int>> v1 = { { 1, 2 }, { 3, 4 } };
	std::vector<vector<int>> v1_expected = { { 1, 2, 3, 4 } };
	REQUIRE( s.matrixReshape(v1, 1, 4) == v1_expected);
	REQUIRE( s.matrixReshape(v1, 2, 4) == v1);
	REQUIRE( s.matrixReshape(v1_expected, 2, 2) == v1);
}
